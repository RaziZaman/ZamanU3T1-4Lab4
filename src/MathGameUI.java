...
public class MathGameUI {
    ...
    private void mainMenu() {
        ...
        while (playAgain.equals("y")) {
            ...
            if (previousWinner.equals(game.getWinner().getName())) {
                winStreak++;
            } else {
                winStreak = 1;
            }
            if (game.getWinner() != null) {  // check for null just to be safe (so we don't call a method on null)
                ...
                if (winStreak == 1) {
                    System.out.println(game.getWinner().getName() + " has won " + winStreak + " time!");
                } else {
                    System.out.println(game.getWinner().getName() + " has won " + winStreak + " games in a row!");
                }
            }
            System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
            previousWinner = String.valueOf(game.getWinner().getName());
